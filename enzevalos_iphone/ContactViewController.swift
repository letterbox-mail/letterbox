...
//        headerCell.layoutMargins = UIEdgeInsetsZero
    ...
    func prepareContactSheet() {
        ...
        if authorizationStatus == CNAuthorizationStatus.Authorized {
            do {
                ui = try AppDelegate.getAppDelegate().contactStore.unifiedContactWithIdentifier(contact!.getContact().getContact()!.identifier, keysToFetch: [CNContactViewController.descriptorForRequiredKeys()])
            } catch {
        ...
        if let conUI = ui {
            ...
            vc = CNContactViewController(forNewContact: contact!.getContact().getContact())
            vc!.contactStore = AppDelegate.getAppDelegate().contactStore // nötig?
            vc!.delegate = self
